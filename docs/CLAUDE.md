...
### **8\. DLL Injection (Planned — Fast Gump Actions)**

* **Problem:** Stealth validates gump buttons client-side. Cross-page BOD book buttons are rejected. Page flipping = ~1s/page.
* **Solution:** DLL injected into Stealth hooks WinSock `send()`. Python sends commands via named pipe. DLL constructs raw 0xB1 packets (23 bytes) with any global button ID and sends on Stealth's existing server socket.
* **0xB1 packet:** `cmd=0xB1 | length=23 | serial(4) | gumpId(4) | buttonId(4) | switchcount=0(4) | textcount=0(4)`
* **Global button formula:** `5 + (global_pos * 2)` — server accepts regardless of displayed page.
* **Gump serial refreshes** after every button press (server sends new gump). Must re-read serial between drops. Expected ~200ms/drop.
* **Confirmed working:** pos 432-434 (page 43) dropped in 1.8s with zero page flips. ~300ms/drop including serial refresh.
* **Socket discovery:** brute-force scan handles 4-10000 (step 4), send ping 0x73, first success = game socket. Handle persists for session.
* **Files:** `uo_packet_injector/` (Rust DLL), `inject_dll.py` (injector), `BodCycler_PacketBridge.py` (Python API), `fast_drop_bods()` in ConservaManager.
* **Debug scripts:** `docs/debug_scripts/_debug_gump_packet.py` (gump behavior), `_debug_packet_bridge.py` (end-to-end DLL test).