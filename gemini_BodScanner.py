...
def infer_material(item_name, current_mat):
    """Infer material from item type if it's missing (Iron)."""
    if current_mat != "Iron":
        return current_mat
...
def parse_page_visually(g):
    ...
    for b in final_bods:
        reward_str = ""
        if b['prize_id'] and b['prize_id'] in prize_names:
            reward_str = f"-> REWARD: {prize_names[b['prize_id']]}"
        # AddToSystemJournal(f"{b['type']} | {b['item']} | {b['quality']} | {b['material']} | {b['amount']} {reward_str}")
...
def generate_progress_report(all_bods):
    ...
    # 2. Define Targets to Report (Set Name, Material, Amount, Quality, Reward Name)
    # Added "Quality" to the tuple so we can handle Normal vs Exceptional
    targets = [
        # Barbed Kits (Exceptional)
        ...
        # CBDs (Normal & Exceptional)
        ("Male Leather Set", "Leather", 20, "Normal", "CBD"),        # <--- NEW
        ("Female Leather Set", "Leather", 20, "Normal", "CBD"),      # <--- NEW
